 ...
 * \param char *mensajeError que ve el usuario despues de ingresar un dato no solicitado o incorrecto
...
/** \brief recibe array y verifica que sea un nombre valido
 * \param array de char
 * \return int(-1) campos NUll int (0) si resuelve
 */
int esNombre(char* pArray)
{
	int retorno=0;
	int i=0;
	if(pArray!=NULL)
	{
		while(pArray[i]!='\0')
		{
		  if(pArray[i]==pArray[0] && pArray[i]=='-' && pArray[i]=='´' && pArray[i]==' ')
		  {
			  retorno=-1;
			  break;
		  }
		  else if((pArray[i]<'a' || pArray[i]>'z') && (pArray[i]<'A' || pArray[i]>'Z')&& pArray[i]!='-' && pArray[i]!=' ' && pArray[i]!='´')
		  {
		  	 retorno=-1;
			 break;
		  }
 ...
 */
int esNumericaFloat(char* pArray)
 {
 	 int retorno=0;
	 int i=0;
 	 int contarPunto=0;
...
/** \brief envia un mensaje de verificacion al usuario para que conteste con S o N (si o no)
 * \param array de char
 * \return int(-1) campos NUll int (1) si resuelve
 */
...
/** \brief recibe un puntero a char y verifica que sea un nombre valido
 * \param Lista estructura
 * \param int Largo
 * \param char *mensaje que ve el usuario antes de ingresar el dato solicitado
 * \param char *mensajeError que ve el usuario despues de ingresar un dato no solicitado o incorrecto
 ...
 * \return int(-1) parametro NULL (0) si resolvio
 */
int utn_getNombre(char* pArray,int LARGO,char* mensaje, char* mensajeError,int reintentos)
{
	int retorno=-1;
	char buffer[51];
	if(pArray!=NULL && LARGO>0 && mensaje!=NULL && mensajeError!=NULL && reintentos>=0)
 ...
 * \return int(-1) campos NUll int (posicion libre) si resuelve
 */
int utn_getNumeroChar(char* pResultado,char* mensaje,char* mensajeError,int minimo,int maximo,int reintentos)
{
	int retorno=-1;
	int bufferInt;
	char array[51];
	if(pResultado!=NULL && mensaje!=NULL && mensajeError!=NULL )
	{
		do
		{
			printf("%s",mensaje);
			fflush(stdin);
			if(myGets(array,sizeof(array))==0 && esNumerica(array)==0)
			{
				bufferInt=atoi(array);
				if (bufferInt>=minimo && bufferInt<=maximo)
				{
					strncpy(pResultado,array,51);
					retorno=0;
					break;
				}
				else
